...
p1 <- ggplot(adra, aes(factor(q19), factor(q20))) + 
 geom_jitter(pch = 20, size = 1, colour = "red",
      position = position_jitter(width = 0.1, height = 0.1)) +
   scale_x_discrete("Extent to which classroom practices have changed", 
      breaks = 0:10, expand = c(0,0.75)) +
   scale_y_discrete("Extent to which you shared what you learned with others", 
      breaks = 0:10, expand = c(0,0.75)) +
   geom_text(x = (0.6 + 2.1)/2, y = (9.1 + 9.9)/2,
      label = paste("r = ", round(cor, 2), sep = ""), size = 4) +
   geom_rect(xmin = .6, xmax = 2.1, ymin = 9.1, ymax = 9.9, colour = "black", fill = NA) +
   theme_bw() +
...
# Horizontal marginal hhistogram - to appear at the top of the chart
p2 <- ggplot(adra, aes(x = factor(q19))) + 
    geom_histogram(stat = "bin", fill = "skyblue") + 
    scale_x_discrete(breaks = 0:10, expand = c(0, 0.75)) +
    theme_bw() + 
    theme(panel.grid.minor = element_blank(),
     panel.grid.major = element_blank(),
...
# Vertical marginal histogram - to appear at the right of the chart
p3 <- ggplot(adra, aes(x = factor(q20))) + 
     geom_histogram(stat = "bin", fill = "skyblue") + 
     scale_x_discrete(breaks = 0:10, expand = c(0, 0.75)) +
     coord_flip() +
     theme_bw() + 
